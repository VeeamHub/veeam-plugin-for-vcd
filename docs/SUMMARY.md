# VEEAM PLUGIN FOR VCD

* [Introduction](README.md)
* [Upgrading the Veeam Plugin](plugin-upgrade-instructions.md)

## VEEAM SELF-SERVICE PORTAL

* [1. VSSP Setup](vssp-setup.md)
* [2. IIS Configuration](vssp-iis-config.md)

## VEEAM PLUGIN DEPLOYMENT

* [3. Deployment Methods](plugin-deployment-methods.md)
  * [Using VMware Plugin Lifecycle Manager](plugin-deploy-plugin-manager.md)
  * [Using Python](plugin-deploy-python.md)

## VEEAM PLUGIN CONFIGURATION

* [4. vCD Provider Portal Configuration](plugin-configuration-provider.md)
* [5. vCD Tenant Portal Usage](plugin-usage-tenant.md)
